...
If the granted bit is set, then userspace has to invoke sched_yield(2) when
...
If the thread issues a syscall other than sched_yield(2) within the granted
timeslice extension, the grant is also revoked and the CPU is relinquished
immediately when entering the kernel. This is required as syscalls might
consume arbitrary CPU time until they reach a scheduling point when the
preemption model is either NONE or VOLUNTARY and therefore might
intentionally try to exceed the grant.

The preferred solution for user space is to use sched_yield(2) which is
side effect free. The support for arbitrary syscalls is required to support
onion layer architectured applications, where the code handling the
